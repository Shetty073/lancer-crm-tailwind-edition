    ...
    <div class="divide-y divide-gray-200">
        <div class="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
            <div class="flex flex-col md:flex-row">
                <div class="flex flex-col mr-4 w-full md:w-3/12">
                    ...
                    <input type="text" name="payer"
                        ...
                        placeholder="Payer Name" value="@if(isset($due)){{ $due->payer }}@else{{ old('payer') }}@endif" required>
                </div>
                <div class="flex flex-col mr-4 w-full md:w-3/12">
                    ...
                    <input type="number" step="0.01" name="amount"
                        class="px-4 py-2 border focus:ring-gray-500 focus:border-indigo-400 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600"
                        placeholder="Amount in {{ App\Lancer\Utilities::CURRENCY_SYMBOL }}" value="@if(isset($due)){{ $due->amount }}@else{{ old('amount') }}@endif" required>
                </div>
                <div class="flex flex-col mr-4 w-full md:w-3/12">
                    <label class="leading-loose capitalize font-semibold text-indigo-600">Due Date</label>
                    <input type="date" name="due_date"
                        class="px-4 py-2 border focus:ring-gray-500 focus:border-indigo-400 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600"
                        value="@if(isset($due)){{ $due->due_date->format('Y-m-d') }}@else{{ Carbon\Carbon::now()->format('Y-m-d') }}@endif" required>
                </div>
                <div class="flex flex-col mr-4 w-full md:w-3/12">
            ...
            <div class="flex flex-col md:flex-row">
                <div class="flex flex-col mr-4 w-full md:w-3/12">
                    ...
                    <input type="text" name="remark"
                        class="px-4 py-2 border focus:ring-gray-500 focus:border-indigo-400 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600"
                        placeholder="Remark" value="@if(isset($due)){{ $due->remark }}@else{{ old('remark') }}@endif" >
                </div>
            </div>
            ...
            <div class="flex flex-row">
                ...
                <a href="{{ route('dues.index') }}" class="w-6/12 md:w-2/12 text-center px-1 py-2 rounded bg-red-500 hover:bg-red-700 text-white focus:outline-none">
                    Cancel
                </a>
            </div>
